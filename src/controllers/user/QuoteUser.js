...
const {
  getAllQuotesUser,
  fetchQuoteByClauseUser,
  fetchQuoteByClause,
} = require("../service/QuoteService");
const {
  getPagination,
  getPagingData,
} = require("../service/PaginationService");
...
const {
  ...
  Uploads,
  Inspections,
  quote_rejected_logs: QuoteRejected,
} = db;
...
exports.findAllQuotesForUser = (req, res) => {
  ...
  const whereClause = {
    ...
    status: { [Op.notIn]: ["CLOSED", "PROJECT_IN_PROGRESS"] },
  };
...
exports.findQuoteById = async (req, res, next) => {
  ...
  if (isOpr) {
    let opCost = quote.dataValues.QuoteOperation.map(
      (dv) => dv.dataValues.operation_cost
    );
    opCost = opCost.map((i) => Number(i));
    let inspId = quote.dataValues.QuoteOperation.map(
      (dv) => dv.dataValues.inspection_id
    );
...
exports.submitPOUrl = async (req, res, next) => {
  ...
  try {
    const boolean = QuoteStatus.checkQuotesStatusCanBeUpdated(
      quote.status,
      status
    );
    if (!boolean) {
    ...
    Quotes.update(
      { submittedPO: submit_PO, status: status },
